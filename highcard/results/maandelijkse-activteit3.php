    ...
    <script src="../../script.responsive.js"></script>
				<?php
					...
					//connectie met sql server maken
						...
						$result1 = mysqli_query($link, 'SELECT Datum FROM Bericht Where YEAR(datum)="'.$jaar.'" AND BedrijfID='.$bedrijf3) or die('Er ging iets mis11111' . mysqli_error($link));
						$result2 = mysqli_query($link, 'SELECT Datum FROM Reactie Where YEAR(datum)="'.$jaar.'" AND BedrijfID='.$bedrijf3) or die('Er ging iets mis2222' . mysqli_error($link));
						$result3 = mysqli_query($link, 'SELECT Shares, Datum FROM Bericht Where YEAR(datum)="'.$jaar.'" AND BedrijfID='.$bedrijf3) or die('Er ging iets mis3333' . mysqli_error($link));
						$result4 = mysqli_query($link, 'SELECT Likes, Datum FROM Bericht Where YEAR(datum)="'.$jaar.'" AND BedrijfID='.$bedrijf3) or die('Er ging iets mis4444' . mysqli_error($link));
...
<div id="wha-main">
	...
	<nav class="wha-nav clearfix">
		...
		<ul class="wha-hmenu">
			...
			echo '<li><a href="maandelijkse-activteit3.php?B1='. $bedrijf1 .'&B2='.$bedrijf2.'&B3='.$bedrijf3.'" class="active">Maandelijkse Activteit Bedrijf 3</a></li>';
			?>
		</ul>
        </div>
    </nav>
